...
export default async function TemplateEditPage({params}: PageProps) {
  const t = await getTranslations('templateEditPage')
  ...
  const usedPlansCount = usedPlans?.length || 0
    ...
    <div className="mx-auto max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      {usedPlansCount > 0 && (
        <section className="rounded-2xl border border-yellow-200 bg-yellow-50 p-4 text-yellow-800 shadow-sm sm:p-5">
          <h2 className="font-bold">{t('usedWarningTitle')}</h2>

          <p className="mt-2 text-sm leading-6">
            {t('usedWarningDescription', {count: usedPlansCount})}
          </p>
          ...
          <div className="mt-4 rounded-xl border border-yellow-200 bg-white/70 p-3">
            ...
            {usedPlansCount > 5 && (
              <p className="mt-2 text-xs text-yellow-700">
                {t('morePlans', {count: usedPlansCount - 5})}
              </p>
            )}
          </div>
        </section>
