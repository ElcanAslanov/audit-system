...
function canManageTemplates(role: string | null) {
  return role === 'admin' || role === 'rehber' || role === 'audit_muavini'
}

function templatePermissionError() {
  return {
    success: false,
 ...
 export async function createTemplate(prevState: any, formData: FormData) {
  ...
  const current = await getCurrentUserAndRole(supabase)

  if (current.error) {
    return { success: false, error: current.error }
  }

  if (!canManageTemplates(current.role)) {
    return templatePermissionError()
  }
