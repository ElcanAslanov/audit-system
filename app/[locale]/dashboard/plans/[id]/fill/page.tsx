...
export default async function FillAuditPage({params}: PageProps) {
  const t = await getTranslations('fillAudit')
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-red-700 shadow-sm">
          <h2 className="text-xl font-black">{t('viewLockedTitle')}</h2>

          <p className="mt-2 text-sm leading-6">
            {t('viewLockedDescription')}
          </p>
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-3xl rounded-2xl border border-yellow-200 bg-yellow-50 p-6 text-center text-yellow-800 shadow-sm">
          <h2 className="text-xl font-black">{t('editLockedTitle')}</h2>

          <p className="mt-2 text-sm leading-6">
            {t('editLockedDescription')}
          </p>
  ...
  if (planTemplatesError) {
    return (
      <div className="p-4 text-red-600 sm:p-6 lg:p-8">
        {t('templateLoadError', {message: planTemplatesError.message})}
      </div>
    )
  }
  ...
  if (planTemplateSectionsError) {
    return (
      <div className="p-4 text-red-600 sm:p-6 lg:p-8">
        {t('sectionsLoadError', {
          message: planTemplateSectionsError.message,
        })}
      </div>
    )
  }
    ...
    return (
      <div className="p-4 sm:p-6 lg:p-8">
        <div className="mx-auto max-w-3xl rounded-2xl border border-red-200 bg-red-50 p-6 text-center text-red-700">
          <h2 className="text-xl font-bold">{t('noTemplateTitle')}</h2>
          <p className="mt-2">{t('noTemplateDescription')}</p>
          <Link
            href="/dashboard/plans"
  ...
  if (questionsError) {
    return (
      <div className="p-4 text-red-600 sm:p-6 lg:p-8">
        {t('questionsLoadError', {message: questionsError.message})}
      </div>
    )
  }
  ...
  if (customQuestionsError) {
    return (
      <div className="p-4 text-red-600 sm:p-6 lg:p-8">
        {t('customQuestionsLoadError', {
          message: customQuestionsError.message,
        })}
      </div>
    )
  }

  const normalizedCustomQuestions = (customQuestions || []).map(
    (question: any) => {
      ...
      return {
        ...
        created_by_name: creator?.full_name || t('auditorFallback'),
        template_sections: {
          id: 'custom',
          title: t('customSectionTitle'),
          sort_order: 999999,
          audit_templates: {
            id: 'custom',
            title: t('customTemplateTitle'),
          },
        },
      }
    }
  ...
  if (findingsError) {
    return (
      <div className="p-4 text-red-600 sm:p-6 lg:p-8">
        {t('findingsLoadError', {message: findingsError.message})}
      </div>
    )
  }
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      {planTemplates && planTemplates.length > 1 && (
        <section className="rounded-2xl border border-indigo-200 bg-indigo-50 p-4 text-indigo-800 shadow-sm sm:p-5">
          <h2 className="font-bold">{t('multiTemplateTitle')}</h2>
          <p className="mt-2 text-sm leading-6">
            {t('multiTemplateDescription', {count: planTemplates.length})}
          </p>
        </section>
      )}
      ...
      {hasAnswers && (
        <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4 text-blue-800 shadow-sm sm:p-5">
          <h2 className="font-bold">{t('editNoticeTitle')}</h2>
          <p className="mt-2 text-sm leading-6">
            {t('editNoticeDescription', {count: answeredCount})}
          </p>
        </section>
      )}
      ...
      {isCompletedOrAttention && (
        <section className="rounded-2xl border border-yellow-200 bg-yellow-50 p-4 text-yellow-800 shadow-sm sm:p-5">
          ...
          <p className="mt-2 text-sm leading-6">
            {t('attentionDescription', {status: statusLabel(plan.status)})}
          </p>
        </section>
      )}
