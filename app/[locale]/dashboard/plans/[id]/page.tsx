...
export default async function AuditDetailPage({params}: PageProps) {
  ...
  const lockLabel = (lockedView: boolean, lockedEdit: boolean) => {
    if (lockedView) return t('viewAndEditLocked')
    if (lockedEdit) return t('editLocked')
    return t('unlocked')
  }
    ...
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6 lg:p-8">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            {hasAnswers ? (
              <>
                ...
                <Link
                  href={`/dashboard/plans/${id}/export-excel?locale=${locale}`}
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-sm font-semibold text-slate-500">{t('status')}</p>
        ...
        <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5 shadow-sm">
          <p className="text-sm font-semibold text-blue-700">{t('score')}</p>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 font-bold text-slate-900">
            {formatDate(plan.start_date)} → {formatDate(plan.due_date)}
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          >
            {lockLabel(isViewLocked, isEditLocked)}
          </p>
        </div>
      </section>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="rounded-xl bg-slate-50 p-3">
                <p className="text-xs text-slate-500">{t('answers')}</p>
              ...
              <div className="rounded-xl bg-red-50 p-3">
                <p className="text-xs text-red-700">{t('problem')}</p>
                        ...
                        return (
                          <>
                            <p className="mt-1 text-xs font-semibold text-blue-600">
                              {template?.title || t('templateFallback')} /{' '}
                              {section?.title || t('sectionFallback')}
                            </p>
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-2 border-b pb-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <span className="w-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600">
                {t('findingsCount', {count: findings?.length || 0})}
              </span>
            </div>
                        ...
                        {finding.files.map((file: any, fileIndex: number) => (
                          ...
                          >
                            ...
                            <div className="flex gap-2">
                              <a
                                href={`/api/finding-files?path=${encodeURIComponent(
                                  file.path
