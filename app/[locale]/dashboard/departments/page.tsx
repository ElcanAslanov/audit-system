  ...
  if (companiesError) {
    return (
      <div className="p-4 text-red-600 sm:p-6 lg:p-8">
        {t('companiesLoadError', {message: companiesError.message})}
      </div>
    )
  }
          ...
          <div className="mb-5 flex flex-col gap-2 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {t('showing', {count: departmentList.length})}
              </p>
            </div>
          </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-2 flex items-center gap-2 text-sm text-slate-500">
                        <Building2 size={15} />
                        {t('company')}: {department.companies?.name || '-'}
                      </p>

                      <p className="mt-1 flex items-center gap-2 text-sm text-slate-500">
                        <CalendarDays size={15} />
                        {t('registration')}: {formatDate(department.created_at)}
                      </p>
                    </div>
                  </div>
