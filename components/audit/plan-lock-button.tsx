...
import { updateAuditPlanLock } from '@/app/[locale]/dashboard/plans/actions'
...
export default function PlanLockButton({
  ...
  const handleLock = (lockType: 'none' | 'edit' | 'view') => {
    ...
    startTransition(async () => {
      const result = await updateAuditPlanLock(planId, lockType, locale)
          ...
          >
            <div className="flex items-start justify-between gap-4 border-b border-slate-100 p-5">
              <div className="flex items-start gap-3">
                <div
                  className={`grid h-12 w-12 shrink-0 place-items-center rounded-2xl ${lockedView
            ...
            <div className="space-y-2 bg-slate-50 p-5">
              ...
              >
                ...
                <span>
                  <span className="block font-black">{t('unlockTitle')}</span>
  ...
  return (
    <>
      <button
        ...
        className={`inline-flex items-center justify-center rounded-full border font-bold transition disabled:cursor-not-allowed disabled:opacity-60 ${compact ? 'h-8 w-8 p-0' : 'w-full gap-2 px-3 py-2.5 text-sm'
          } ${lockedView
