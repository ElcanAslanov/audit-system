...
export default function CompareResultSection({
  result,
  statusLabel,
  answerLabel,
}: {
  ...
  answerLabel: (value?: string | null) => string
}) {
  ...
  const highLeft = riskValue(result.left, 'high')
  const highRight = riskValue(result.right, 'high')

  const mediumLeft = riskValue(result.left, 'medium')
  const mediumRight = riskValue(result.right, 'medium')

  const lowLeft = riskValue(result.left, 'low')
  const lowRight = riskValue(result.right, 'low')

  const problemLeft = problemAnswers(result.left)
  const problemRight = problemAnswers(result.right)

  const questionRows = useMemo(
    () => buildQuestionRows(result.left, result.right, t('questionFallback')),
    [result.left, result.right, t]
  )
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-2 text-sm">
            ...
            <p>
              <span className="text-slate-500">{t('checklistAnswers')}:</span>{' '}
                  ...
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <p className="text-xs font-bold uppercase tracking-wide text-slate-400">
                        #{index + 1} • {row.templateTitle} / {row.sectionTitle}
                      </p>
